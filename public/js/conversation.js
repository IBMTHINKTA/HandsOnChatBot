      ...
      var reader = new FileReader();
        reader.onload = function () {
          ...
          image.onload = function () {
            var resizedImage = (file['size'] > 300000) ? resize(image, 400) : this.src;
            var context;
            var latestResponse = Api.getResponsePayload();
            if (latestResponse) {
              context = latestResponse.context;
            }
              Api.sendRequestPic(resizedImage, context);
          }
          var chatBoxElement = document.getElementById("scrollingChat");
            var currentDiv = 
              "<div class=\"loading-wrapper segments load \" >"+
                "<div class=\"from-user latest top\">"+
                  "<div class=\"message-inner\"><img src=\""+image.src+"\" style=\"width:auto; height:10%; vertical-align:bottom; margin:0rem ;\">"+
                  "</div>"+
                "</div>"+
              "</div>";
